...
class Validate {
    ...
    public $row;
    public $result;
    public $valid = [ 'validEmail' => true, 'validPassword' => true, 'duplicate' => true, 'empty' => true];
    protected $data = [];
    public $resultArray = [];
    public $message = '';
    ...
    public function contentCheck() {
        ...
        $this->usernameCheck($this->data['name']);
        $this->passwordCheck($this->data['password']);
        $this->emailCheck($this->data['email']);
        return $this->validate();
    }

    protected function usernameCheck($name) {
        $db = DB::getInstance();
        $pdo = $db->getConnection();
        try {
            $this->query = "SELECT 1 FROM tracker WHERE name = :name";
